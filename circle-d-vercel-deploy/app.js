const dataset = window.CONVERTER_DATA;

if (!dataset || !dataset.families?.length) {
  throw new Error("Converter data bundle missing. Run scripts/build_data.py first.");
}
...
function renderStats() {
  ...
  refs.converterStats.innerHTML = stats
    .map(
      ([label, value]) =>
...
function renderCharts() {
  ...
  state.charts.kFactor = buildChart(
    refs.kFactorCanvas,
    converter.name,
...
function renderPrevNext() {
  ...
  refs.prevConverter.onclick = previous
    ? () => setSelection(state.selectedFamilyId, previous.id)
    : null;
...
function initCounts() {
  refs.familyCount.textContent = dataset.meta.familyCount;
  refs.converterCount.textContent = dataset.meta.converterCount;
}
...
function initState() {
  ...
  const defaultConverter =
    defaultFamily.converters.find((converter) => converter.id === route.converter) ||
    defaultFamily.converters[0];
...
function init() {
  ...
  window.addEventListener("hashchange", () => {
    ...
    const family = state.families.find((item) => item.id === route.family);
    if (!family) return;
    const converter = family.converters.find((item) => item.id === route.converter) || family.converters[0];
    state.selectedFamilyId = family.id;
    state.selectedConverterId = converter.id;
    render();
  });
}
